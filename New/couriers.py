...
def add_courier_to_list(new_id, name, number):
    ...
    with connection.cursor(pymysql.cursor.DictCursor) as cursor:
        ...
        sql = "INSERT INTO Couriers ({}, {}, {}) VALUES (%s, %s, %s)".format( new_id, name, number)
        val = ("{}".format(new_id), "{}".format(name), "{}".format(number))
...
def update_name(select):
    ...
    # database.update_to_table(table,name, new_name, name, old_name)

    with connection.cursor(pymysql.cursor.DictCursor) as cursor:
        sql = "UPDATE Couriers SET name = '{}' WHERE id = '{}'".format(new_name, the_id)
...
def update_number(selected):
    ...
    if new_number != "":
        ...
        with connection.cursor(pymysql.cursor.DictCursor) as cursor:
            sql = "UPDATE Couriers SET number = '{}' WHERE id = '{}'".format(
                new_number, the_id)
...
def removal_of_courier():
    ...
    with connection.cursor(pymysql.cursor.DictCursor) as cursor:
        sql = "DELETE FROM Couriers WHERE id = '{}'".format(the_id)
