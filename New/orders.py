...
def add_new_order(o_id,name,address,number,courier,items):
    ...
    with connection.cursor(pymysql.cursor.DictCursor) as cursor:
        sql = "INSERT INTO Orders ({}, {}, {}, {}, preparing, {}) VALUES (%s, %s,  %s, %s, %s)".format(
            name, address, number, courier, items)
        val = ("{}".format(name), "{}".format(address), "{}".format(number),
            "{}".format(courier), "{}".format(items))
...
def update_status(selected):
    ...
    with connection.cursor(pymysql.cursor.DictCursor) as cursor:
        sql = "UPDATE Orders SET status = '{}' WHERE id = '{}'".format(
            status, the_id)
...
def update_name(selected_order):
    ...
    if new_name != "":
        ...
        with connection.cursor(pymysql.cursor.DictCursor) as cursor:
            sql = "UPDATE Order SET name = '{}' WHERE id = '{}'".format(new_name, the_id)
...
def update_address(selected_order):
    ...
    if new_address != "":
        ...
        with connection.cursor(pymysql.cursor.DictCursor) as cursor:
            sql = "UPDATE Orders SET address = '{}' WHERE id = '{}'".format(
                new_address, the_id)
...
def update_phone_number(selected_order):
    ...
    if new_number != "":
        ...
        with connection.cursor(pymysql.cursor.DictCursor) as cursor:
            sql = "UPDATE Orders SET number = '{}' WHERE id = '{}'".format(
                new_number, the_id)
...
def update_courier(selected_order):
    ...
    try:
        ...
        if picked_courier != "":
            ...
            with connection.cursor(pymysql.cursor.DictCursor) as cursor:
                sql = "UPDATE Orders SET courier = '{}' WHERE id = '{}'".format(
                    index, the_id)
...
def update_product_part_4(selected_order):
    ...
    if update_product_part_3() != False:
        ...
        with connection.cursor(pymysql.cursor.DictCursor) as cursor:
            sql = "UPDATE Orders SET item = '{}' WHERE id = '{}'".format(new_items, the_id)
...
def remove_order():
    ...
    with connection.cursor(pymysql.cursor.DictCursor) as cursor:
        sql = "DELETE FROM Orders WHERE id = '{}'".format(the_id)
